...
export function LeadForm({ compact = false, title = 'Request a private preview', subtitle, submitLabel = 'Enquire now', onSuccess }: Props) {
  ...
  return <form onFocus={()=>{if(!started){setStarted(true);trackEvent('form_start')}}} onSubmit={handleSubmit(onSubmit)} className={`rounded-[18px] border border-gold/35 bg-[#fbf8f3]/95 p-5 text-ink shadow-luxury backdrop-blur-xl ${compact ? 'md:p-5' : 'md:p-8'}`} noValidate>
    ...
    <div className={compact ? 'grid gap-x-5 md:grid-cols-2' : 'space-y-3'}>
